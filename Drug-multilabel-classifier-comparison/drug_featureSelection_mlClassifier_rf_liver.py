...
def br_feature_selection_and_train(X, y, organ_choice, feat_select):
    ...
    for feature_selected in range(1, feat_select+1, 1):
        ...
        list_1 = [organ_choice, 'br', feature_selected, max_n_estimators, max_depth, max_ACC, max_TPR, max_SPE, max_F1, max_AUC,
                  max_Pairwise_Acc, max_Average_Label_Accuracy] + max_Label_Acc_list
        pd_data = pd.DataFrame(np.array(list_1).reshape(1, 24))
...
def cc_feature_selection_and_train(X, y, organ_choice, feat_select):
    ...
    for feature_selected in range(1, feat_select+1, 1):
        ...
        # Writing CSV Files
        # columns=['organ_choice', 'clf', 'feat_num', 'C', 'ACC', 'TPR', 'spe', 'f1', 'auc']
        list_1 = [organ_choice, 'cc', feature_selected, max_n_estimators, max_depth, max_ACC, max_TPR, max_SPE, max_F1, max_AUC,
                  max_Pairwise_Acc, max_Average_Label_Accuracy] + max_Label_Acc_list
        pd_data = pd.DataFrame(np.array(list_1).reshape(1, 24))
...
if __name__ == '__main__':
    ...
    """
    organ_choice = "liver"
    dataset = pd.read_csv('dataset/liver-MLFS-sorted-feature.csv')
    print("dataset.shape: " + str(dataset.shape))

    # split the features-X and class labels-y
    X = dataset.iloc[:, 12:]  # features
    y = dataset.iloc[:, :12].astype('int')   # labels
    feat_select = 200
