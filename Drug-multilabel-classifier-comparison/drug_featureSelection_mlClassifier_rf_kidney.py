...
def br_feature_selection_and_train(X, y, organ_choice, feat_select):
    ...
    for feature_selected in range(1, feat_select+1, 1):
        ...
        # Parameter tuning. base models: RandomForestClassifier()
        # n_estimators_list = [10]
        # max_depth_list = [3]
        n_estimators_list = [10, 20, 30, 40]
        max_depth_list = [2, 3]
        ...
        for n in n_estimators_list:
            for d in max_depth_list:
                ...
                for train_index, test_index in kf.split(XX, y):
                    ...
                    clf = OneVsRestClassifier(RandomForestClassifier(n_estimators=n, max_depth=d, random_state=1))
                    # train
                    ...
                    for i in range(0, label_num, 1):
                        lbl_pred = clf.estimators_[i].predict(
                            x_val)  # the i-th label prediction, each label predict value
        ...
        print(organ_choice, 'br', 'feature number:', feature_selected, 'n:', max_n_estimators, 'd:', max_depth,
        ...
        list_1 = [organ_choice, 'br', feature_selected, max_n_estimators, max_depth, max_ACC, max_TPR, max_SPE, max_F1,
                  max_AUC, max_Pairwise_Acc, max_Average_Label_Accuracy] + max_Label_Acc_list
        pd_data = pd.DataFrame(np.array(list_1).reshape(1, 20))
        output_path = "./output/" + organ_choice + "_br_randomForest_validation_result.csv"
        ...
        if max_ACC in mydict_d.keys():
            print("key already exists")
        else:
            mydict_d[max_ACC] = max_depth
        if max_ACC in mydict_n.keys():
            print("key already exists")
        else:
            mydict_n[max_ACC] = max_n_estimators

    final_feature_selected = mydict_ACC[max(final_ACC)]  # max acc and the smallest feature set
    final_n = mydict_n[max(final_ACC)]
    final_d = mydict_d[max(final_ACC)]

    return final_feature_selected, final_n, final_d
...
def cc_feature_selection_and_train(X, y, organ_choice, feat_select):
    ...
    for feature_selected in range(1, feat_select+1, 1):
        ...
        for n in n_estimators_list:
            for d in max_depth_list:
                ...
                for train_index, test_index in kf.split(XX, y):
                    ...
                    x_train, x_val, y_train, y_val = train_test_split(X_train, Y_train, test_size=0.2, random_state=1)
                    # Initialize the classifier to be used
                    clf = ClassifierChain(RandomForestClassifier(n_estimators=n, max_depth=d, random_state=1))
                    # train
                    clf.fit(x_train, y_train)
                    # y_prediction
                    y_pred = clf.predict(x_val)
        ...
        print(organ_choice, 'cc', 'feature number:', feature_selected, 'n:', max_n_estimators, 'd:', max_depth,
              'ACC:', max_ACC, 'TPR:', max_TPR, 'SPE', max_SPE, 'F1_score:', max_F1, 'AUC:', max_AUC,
              'max_Pairwise_Acc:', max_Pairwise_Acc, 'Ave_Label_Acc:', max_Average_Label_Accuracy)

        # Writing CSV Files
        # columns=['organ_choice', 'clf', 'feat_num', 'C', 'ACC', 'TPR', 'spe', 'f1', 'auc']
        list_1 = [organ_choice, 'cc', feature_selected, max_n_estimators, max_depth, max_ACC, max_TPR, max_SPE, max_F1,
                  max_AUC, max_Pairwise_Acc, max_Average_Label_Accuracy] + max_Label_Acc_list
        pd_data = pd.DataFrame(np.array(list_1).reshape(1, 20))
        output_path = "./output/" + organ_choice + "_cc_randomForest_validation_result.csv"
        pd_data.to_csv(output_path, mode='a+', header=False)
...
def br_random_forest(X, y, organ_choice, feat_select):
    # K-Folds cross-validator
    kf = KFold(n_splits=5)

    # best parameter
    feature_selected, n, d_value = br_feature_selection_and_train(X, y, organ_choice, feat_select)
    ...
    print("test:" + str(ACC))
    print(np.mean(ACC2))
    print(organ_choice, 'br', 'feature number:', feature_selected, 'n:', n, 'd:', d_value,
    ...
    # Writing CSV Files
    columns_name = ['organ_choice', 'clf', 'feat_num', 'n', 'd', 'ACC', 'TPR', 'spe', 'f1', 'auc', 'pairAcc',
                    'aveLabAcc', 'label1', 'label2', 'label3', 'label4', 'label5', 'label6', 'label7', 'label8']
    list_1 = [organ_choice, 'br', feature_selected, n, d_value, np.mean(ACC), np.mean(TPR), np.mean(SPE),
    ...
    output_path = "./output/" + organ_choice + "_br_randomForest_test_result.csv"
    pd_data.to_csv(output_path, index=False)


# ## Classifier Chains Algorithm validation set
def cc_random_forest(X, y, organ_choice, feat_select):
    # K-Folds cross-validator
    kf = KFold(n_splits=5)

    # best parameter
    feature_selected, n, d_value = cc_feature_selection_and_train(X, y, organ_choice, feat_select)
    ...
    print("test:" + str(ACC))
    print(np.mean(ACC2))
    print(organ_choice, 'cc', 'feature number:', feature_selected, 'n:', n, 'd:', d_value,
          'ACC:', np.mean(ACC), 'TPR:', np.mean(TPR), 'SPE', np.mean(SPE), 'F1_score:', np.mean(F1_score),
          'AUC:', mean_auc, 'Pairwise_Acc:', np.mean(Pairwise_Acc), 'Ave_Label_Acc:', np.mean(Average_Label_Accuracy))
    ...
    columns_name = ['organ_choice', 'clf', 'feat_num', 'n', 'd', 'ACC', 'TPR', 'spe', 'f1', 'auc', 'pairAcc',
                    'aveLabAcc', 'label1', 'label2', 'label3', 'label4', 'label5', 'label6', 'label7', 'label8']
    list_1 = [organ_choice, 'cc', feature_selected, n, d_value, np.mean(ACC), np.mean(TPR), np.mean(SPE),
              np.mean(F1_score), mean_auc, np.mean(Pairwise_Acc), np.mean(Average_Label_Accuracy)] + mean_Label_Acc
    pd_data = pd.DataFrame(np.array(list_1).reshape(1, 20), columns=columns_name)
    output_path = "./output/" + organ_choice + "_cc_randomForest_test_result.csv"
    pd_data.to_csv(output_path, index=False)
...
if __name__ == '__main__':
    ...
    br_random_forest(X, y, organ_choice, feat_select)
    cc_random_forest(X, y, organ_choice, feat_select)
