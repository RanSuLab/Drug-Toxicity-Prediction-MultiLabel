...
def br_feature_selection_and_train(X, y, organ_choice, feat_select):
    ...
    for feature_selected in range(1, feat_select+1, 1):
        ...
        # Parameter tuning. base models: LogisticRegression()
        ...
        pd_data = pd.DataFrame(np.array(list_1).reshape(1, 23))
        output_path = "./output/" + organ_choice + "_br_LogisticRegression_validation_result.csv"
...
def cc_feature_selection_and_train(X, y, organ_choice, feat_select):
    ...
    for feature_selected in range(1, feat_select+1, 1):
        ...
        # Parameter tuning. base models: LogisticRegression()
        c_value_list = [0.001, 0.01, 0.1, 1, 10, 100]
        # c_value_list = [1]
        ...
        list_1 = [organ_choice, 'cc', feature_selected, max_c_value, max_ACC, max_TPR, max_SPE, max_F1, max_AUC,
                  max_Pairwise_Acc, max_Average_Label_Accuracy] + max_Label_Acc_list
        pd_data = pd.DataFrame(np.array(list_1).reshape(1, 23))
        output_path = "./output/" + organ_choice + "_cc_LogisticRegression_validation_result.csv"
        pd_data.to_csv(output_path, mode='a+', header=False)
...
def br_logistic_regression(X, y, organ_choice, feat_select):
    # K-Folds cross-validator
    kf = KFold(n_splits=5)
    ...
    output_path = "./output/" + organ_choice + "_br_LogisticRegression_test_result.csv"
    pd_data.to_csv(output_path, index=False)


# ## Classifier Chains Algorithm validation set
def cc_logistic_regression(X, y, organ_choice, feat_select):
    # K-Folds cross-validator
    kf = KFold(n_splits=5)
    ...
    list_1 = [organ_choice, 'cc', feature_selected, c_value, np.mean(ACC), np.mean(TPR), np.mean(SPE),
              np.mean(F1_score), mean_auc, np.mean(Pairwise_Acc), np.mean(Average_Label_Accuracy)] + mean_Label_Acc
    pd_data = pd.DataFrame(np.array(list_1).reshape(1, 23), columns=columns_name)
    output_path = "./output/" + organ_choice + "_cc_LogisticRegression_test_result.csv"
    pd_data.to_csv(output_path, index=False)
...
if __name__ == '__main__':
    ...
    br_logistic_regression(X, y, organ_choice, feat_select)
    cc_logistic_regression(X, y, organ_choice, feat_select)
